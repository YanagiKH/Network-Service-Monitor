...
class 連線表模型(QAbstractTableModel):
    ...
    def 設定欄位標題(self, 標題: list[str]) -> None:
        if len(標題) == len(self._欄位名稱):
            self._欄位名稱 = 標題
            self.headerDataChanged.emit(Qt.Orientation.Horizontal, 0, len(標題) - 1)
